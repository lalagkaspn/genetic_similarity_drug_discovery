...
for(i in 1:nsplit){
  ...
  deb = mult_split_eval_IND(diff, 0, c(sd_man), c(om), di_pairs[splits==i])
  results[[i]]= deb[[4]]
}

bound = bind_rows(results, .id = "column_label")
roc(bound$label, bound$p)
save_path = paste0("results/IND_STAN_PhenoDissimilar_SD=", sd_man, "_OMULT=", om, "_TestPred.csv", collapse = "")
