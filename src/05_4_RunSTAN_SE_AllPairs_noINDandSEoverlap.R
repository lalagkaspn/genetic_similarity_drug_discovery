...
drug_se_both = drug_se_both %>% 
  ...
  dplyr::select(drug, drug_indication = drug_side_effect, drug_indication_category = drug_side_effect_category, phenotype, phenotype_category, drug_indicated_for_pheno = is_side_effect, ldsc_score, magma_spearman_correlation, smultixcan_spearman_cor, ot_l2g_spearman_cor, ot_coloc_overlap_coefficient) %>%
  as.data.frame()
...
save_path = paste0("results/SE_STAN_AllPairs_noINDandSEoverlap_SD=", sd_man, "_OMULT=", om, "_TestPred.csv", collapse = "")
