...
drug_se_both$same_body_system_ClinGraph = ifelse(drug_se_both$cosine_similarity_mean >= -0.1, "Same", "Different")
## consensus
...
drug_se_both$same_body_system_consensus = ifelse(
  drug_se_both$same_body_system_ClinGraph == "Same" | drug_se_both$same_body_system_consensus == "Same", 
  "Same", 
  "Different")
table(drug_se_both$same_body_system_consensus)
drug_se_both = drug_se_both %>% dplyr::select(
  drug, drug_indication = drug_side_effect, drug_indication_category = drug_side_effect_category, phenotype, phenotype_category, drug_indicated_for_pheno = is_side_effect, ldsc_score:ot_coloc_overlap_coefficient, same_body_system_consensus
)
...
## run stan
pair_names = paste(diff$drug, diff$phenotype, sep=":")
...
for(i in 1:nsplit){
  ...
  deb = mult_split_eval_SE(diff, 0, c(sd_man), c(om), di_pairs[splits==i])
  results[[i]]= deb[[4]]
}

bound = bind_rows(results, .id = "column_label")
roc(bound$label, bound$p)
save_path = paste0("results/SE_STAN_PhenoDissimilar_SD=", sd_man, "_OMULT=", om, "_TestPred.csv", collapse = "")
