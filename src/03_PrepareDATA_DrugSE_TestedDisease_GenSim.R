...
#### Purpose: to create a dataframe with all drug-known side effects and tested phenotype pairs with the corresponding genetic similarity metrics
...
## drug side effects
drugs_side_effects = fread("/project/pi_rachel_melamed_uml_edu/Panos/CD_CD_shared_genetics_therapeutics/data/nSIDES/onSIDES_processed.txt") %>%
  dplyr::select(chembl_id, mesh_heading = mesh_name, mesh_id) %>%
  distinct()
# add drug names
chembl_drug_names = fread("/project/pi_rachel_melamed_uml_edu/Panos/CD_CD_shared_genetics_therapeutics/data/opentargets/processed/drug_annotations.txt") %>%
  dplyr::select(chembl_id = "id", name = name, type = drugType) %>%
  distinct()
drugs_side_effects = drugs_side_effects %>%
  left_join(chembl_drug_names, by = "chembl_id") %>%
  dplyr::select(chembl_id, name, type, mesh_heading, mesh_id)
rm(chembl_drug_names)
# manually fix names and types
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1586"), "name"] = "BECLOMETHASONE"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1586"), "type"] = "Small molecule"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL4594371"), "name"] = "CEFTAROLINE"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL4594371"), "name"] = "Small molecule"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL2109882"), "name"] = "TRISILICATE"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL2109882"), "name"] = "Small molecule"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1200709"), "name"] = "CROTAMITON"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1200709"), "name"] = "Small molecule"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1498"), "name"] = "DESOXYCORTICOSTERONE"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1498"), "name"] = "Small molecule"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1180003"), "name"] = "CETRIMIDE"
drugs_side_effects[which(drugs_side_effects$chembl_id == "CHEMBL1180003"), "name"] = "Small molecule"
drugs_side_effects = na.omit(drugs_side_effects)
rownames(drugs_side_effects) = NULL
...
## ==================================================================================================================== ##
## Create a dataframe with drug - known side effects - novel phenotype triplets and  their genetic similarity metrics   ##
## ==================================================================================================================== ##

## drug side effects in our sample
drugs_treating_gwas = drugs_side_effects %>% 
...
drug_gwas_pairs = drug_gwas_pairs %>%
  left_join(drugs_side_effects[, c("chembl_id", "mesh_id")], by = c("drug" = "chembl_id")) %>%
  dplyr::relocate(drug_side_effect = mesh_id, .after = drug) %>%
  filter(drug_side_effect %in% gwas_matching_sources$mesh_id) %>%
  left_join(gwas_matching_sources[,c(1,3)], by = c("drug_side_effect" = "mesh_id")) %>%
  dplyr::relocate(drug_side_effect_category = trait_category, .after = drug_side_effect) %>%
...
## add column to indicate if it is a known side effect (1) or not (0)
drugs_side_effects$is_side_effect = 1
drug_gwas_pairs = left_join(drug_gwas_pairs, unique(drugs_side_effects[, c("chembl_id", "mesh_id", "is_side_effect")]), by = c("drug" = "chembl_id", "phenotype" = "mesh_id"))
drug_gwas_pairs$is_side_effect = ifelse(is.na(drug_gwas_pairs$is_side_effect), 0, 1)

## add genetic similarity metrics
# LDSC-based metric
drug_gwas_pairs = drug_gwas_pairs %>% 
  left_join(ldsc_scores[, c("id1_mesh_id", "id2_mesh_id", "rg")], by = c("drug_side_effect" = "id1_mesh_id", "phenotype" = "id2_mesh_id")) %>%
  left_join(ldsc_scores[, c("id1_mesh_id", "id2_mesh_id", "rg")], by = c("drug_side_effect" = "id2_mesh_id", "phenotype" = "id1_mesh_id")) %>% 
  group_by(drug_side_effect, phenotype) %>%
  mutate(ldsc_score = max(rg.x, rg.y, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(ldsc_score = if_else(ldsc_score == -Inf, NA, ldsc_score)) %>%
  dplyr::select(-rg.x, -rg.y)
# MAGMA-based metric
drug_gwas_pairs = left_join(drug_gwas_pairs, magma_pvalues_spearman_cor_all, by = c("drug_side_effect" = "Disease1_mesh_id", "phenotype" = "Disease2_mesh_id"))
# S-MultiXcan-based metric
drug_gwas_pairs = left_join(drug_gwas_pairs, smultixcan_spearman_cor_all, by = c("drug_side_effect" = "Disease1_mesh_id", "phenotype" = "Disease2_mesh_id"))
# L2G-based metric
drug_gwas_pairs = left_join(drug_gwas_pairs, ot_l2g_spearman_cor_all, by = c("drug_side_effect" = "Disease1_mesh_id", "phenotype" = "Disease2_mesh_id"))
# COLOC-based metric
drug_gwas_pairs = drug_gwas_pairs %>% 
  left_join(ot_gwas_coloc_overlap_coeff[, c("study1_mesh_id", "study2_mesh_id", "ot_coloc_overlap_coefficient")], by = c("drug_side_effect" = "study1_mesh_id", "phenotype" = "study2_mesh_id")) %>%
  left_join(ot_gwas_coloc_overlap_coeff[, c("study1_mesh_id", "study2_mesh_id", "ot_coloc_overlap_coefficient")], by = c("drug_side_effect" = "study2_mesh_id", "phenotype" = "study1_mesh_id")) %>%
  group_by(drug_side_effect, phenotype) %>%
...
drug_gwas_pairs = left_join(drug_gwas_pairs, clingraph_cosine_df, by = c("drug_side_effect" = "id1_mesh", "phenotype" = "id2_mesh"))
# Open Targets same/different body systems
drug_gwas_pairs$same_body_system_OT = ifelse(drug_gwas_pairs$drug_side_effect_category == drug_gwas_pairs$phenotype_category, "Same", "Different")
# UK HRCS and ICD10
...
drug_gwas_pairs = drug_gwas_pairs %>%
  ...
  dplyr::select(drug, drug_side_effect, drug_side_effect_category, drug_side_effect_category_HRCS = trait_area_HRCS.x, drug_side_effect_category_ICD10 = trait_ICD10_top.x,
                phenotype, phenotype_category, phenotype_category_HRCS = trait_area_HRCS.y, phenotype_category_ICD10 = trait_ICD10_top.y,
                is_side_effect, ldsc_score, magma_spearman_correlation, smultixcan_spearman_cor, ot_l2g_spearman_cor, ot_coloc_overlap_coefficient,
                cosine_similarity_mean, same_body_system_OT) %>%
  mutate(
    same_body_system_HRCS = if_else(drug_side_effect_category_HRCS == phenotype_category_HRCS, "Same", "Different"),
    same_body_system_ICD10 = if_else(drug_side_effect_category_ICD10 == phenotype_category_ICD10, "Same", "Different")
  )
...
drug_gwas_pairs = drug_gwas_pairs %>% dplyr::select(
  drug, drug_side_effect, drug_side_effect_category, phenotype, phenotype_category, is_side_effect:ot_coloc_overlap_coefficient, same_body_system_consensus, cosine_similarity_mean
)

# remove rows where the drug indication is the same as the tested disease
drug_gwas_pairs = drug_gwas_pairs %>%
  filter(drug_side_effect != phenotype)

## save
fwrite(drug_gwas_pairs, "data/DrugSE_TestedDisease_GenSim.txt.gz", sep = "\t", row.names = FALSE)
